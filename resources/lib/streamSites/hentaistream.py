...
BASE_URL = 'http://www.hentaistream.com'

base_url_name = BASE_URL.split('www.')[1]
base_txt = base_url_name + ': '

# aniUrls = ['http://www.hentaistream.com/genres/Uncensored/']
aniUrls = []
aniUrls.append(['http://www.hentaistream.com/genres/Uncensored/','hentai'])
...
def Episode_Listing(url):
	...
	match=re.compile('<p class="posttitle"><a href="(.+?)" title="Watch (.+?) Episode (.+?)"><ins>').findall(link)
...
def Total_Video_List(link):
	...
	searchRes = []
	match=re.compile('<h3 class="title"><a(.+?)>(.+?)</a>').findall(link)
	if(len(match) >= 1):
